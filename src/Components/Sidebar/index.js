...
function Sidebar({ currentDataOfUser, active, open, ref }) {
  ...
  function getNameOfUser() {
    let result = JSON.parse(localStorage.getItem("allUsers"))?.find((x) => {
      return x.uid === JSON.parse(localStorage.getItem("user")).uid;
    });
    if (result) {
  ...
  return (
    ...
    >
      <div className="logo_content">
        <div className="logo">
          <div className="logo_name">life</div>
        </div>
      </div>
      <ul className="nav_list">
        ...
        <li>
          <Link to="/blocked-user">
            <TbMessageReport />
            <span className="links_name">Blocked User</span>
          </Link>
          <span className="tooltip">Blocked User</span>
        </li>
      </ul>
